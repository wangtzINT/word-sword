...
3.0.0, but {!r:s} read!".format(value))
    ...
    def get_ifo(self, key):
        ...
        Return:
        - configuration value corresponding to the specified key if exists,
          otherwise False.
        """
...
list and a dictionary.
    The list contains each entry in the .idx file, with subscript indicating the
entry's origin index in .idx file.
    The dictionary is indexed by word name, and the value is an integer or a
list of integers pointing to
...
self._content[self._offset:self._offset+4])
    ...
    def get_index_by_num(self, number):
        """Get index infomation of a specified entry in .idx file by origin
index.
...
class SynFileReader(object):
    ...
    {synonym_word: original_word_index}, in which 'original_word_index' could be
a integer or
    a list of integers.
...
original_word_index]
    ...
    def get_syn(self, synonym_word):
        ...
        Return:
        If synonym_word exists in the .syn file, return the corresponding
